(function () {
    ...
    if (!baseUrl) {
        baseUrl = "https://vocentra-production.up.railway.app";
    }

    // ─── Inject Stylesheet ───────────────────────────────────────────────────
    const cssUrl = script?.src && script.src.startsWith("http") && !script.src.includes(window.location.host)
        ? new URL("assistant.css", script.src).href
        : "/assistant.css";
